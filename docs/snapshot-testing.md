...
Status: v0.5.0 snapshot protocol.
...
2. change expected base mode or modifiers;
...
The v0.5.0 runner validates that the snapshot directory policy file exists, a snapshot file exists for every fixture, and each snapshot file contains expected sections without placeholder content. A full automated model-output runtime execution harness remains deferred.

## Boundary

Snapshot protocol in v0.5.0 is a release-readiness aid. It is not a full automated model-output harness.
